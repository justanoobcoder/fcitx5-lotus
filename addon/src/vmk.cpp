...
std::string buildSocketPath(const char *base_path_suffix) {
    ...
    std::string path =
        "vmksocket-" + username + "-" + std::string(base_path_suffix);
    return path;
}
...
static void DeletePreviousNChars(fcitx::InputContext *ic, size_t n,
...
class VMKState final : public InputContextProperty {
  public:
    ...
    bool connect_uinput_server() {
        ...
        socklen_t len =
            offsetof(struct sockaddr_un, sun_path) + current_path.length() + 1;
    ...
    bool handleUInputKeyPress(fcitx::KeyEvent &event,
                              fcitx::KeySym currentSym) {
    ...
    void replayBufferToEngine(const std::string &buffer) {
        ...
        for (size_t i = 0; i < buffer.length();) {
            if (i + 2 < buffer.length() && buffer.substr(i, 3) == "\\b\\") {
                EngineProcessKeyEvent(vmkEngine_.handle(), FcitxKey_BackSpace,
                                      0);
                i += 3;
            } else {
    ...
    void keyEvent(KeyEvent &keyEvent) {
        ...
        if (E == 1) {
            ...
            if (!ic)
                return;
            EngineProcessKeyEvent(vmkEngine_.handle(), keyEvent.rawKey().sym(),
                                  keyEvent.rawKey().states());
            if (auto commitF =
                    UniqueCPtr<char>(EnginePullCommit(vmkEngine_.handle()));
                commitF && commitF.get()[0]) {
            ...
            if (preeditStr != oldPreBuffer_) {
                ...
                if (!oldPreBuffer_.empty()) {
                    DeletePreviousNChars(ic, fcitx::utf8::length(oldPreBuffer_),
                                         engine_->instance());
                }
                if (!preeditStr.empty()) {
...
vmkEngine::vmkEngine(Instance *instance)
    : instance_(instance),
      factory_([this](InputContext &ic) { return new VMKState(this, &ic); }) {
    startMonitoringOnce();
    Init();
    {
    ...
    auto fd = StandardPath::global().open(StandardPath::Type::PkgData,
                                          "vmk/vietnamese.cm.dict", O_RDONLY);
    if (!fd.isValid())
    ...
    connections_.emplace_back(
        capitalizeMacroAction_->connect<SimpleAction::Activated>(
            [this](InputContext *ic) {
    ...
    uiManager.registerAction("vmk-autonvnrestore",
    ...
    connections_.emplace_back(
        modernStyleAction_->connect<SimpleAction::Activated>(
            [this](InputContext *ic) {
...
std::string vmkEngine::subMode(const fcitx::InputMethodEntry &,
                               fcitx::InputContext &) {
    return *config_.inputMethod;
}

void vmkEngine::activate(const InputMethodEntry &entry,
...
void vmkEngine::keyEvent(const InputMethodEntry &entry, KeyEvent &keyEvent) {
    ...
    if (isSelectingAppMode_ &&
        g_mouse_clicked.load(std::memory_order_relaxed)) {
        closeAppModeMenu();
        ic->inputPanel().reset();
        ic->updateUserInterface(UserInterfaceComponent::InputPanel);
        auto state = ic->propertyFor(&factory_);
...
void vmkEngine::updateInputMethodAction(InputContext *ic) {
    ...
    if (ic) {
        inputMethodAction_->setLongText(
            stringutils::concat("Input Method: ", *config_.inputMethod));
...
void vmkEngine::updateSpellAction(InputContext *ic) {
    ...
    spellCheckAction_->setShortText(
        *config_.spellCheck ? _("Spell Check: Bật") : _("Spell Check: Tắt"));
    if (ic) {
...
void vmkEngine::updateCapitalizeMacroAction(InputContext *ic) {
    ...
    capitalizeMacroAction_->setShortText(*config_.capitalizeMacro
                                             ? _("Capitalize Macro: Bật")
...
void vmkEngine::updateAutoNonVnRestoreAction(InputContext *ic) {
    ...
    autoNonVnRestoreAction_->setShortText(*config_.autoNonVnRestore
                                              ? _("Auto Non-VN Restore: Bật")
...
void vmkEngine::updateModernStyleAction(InputContext *ic) {
    ...
    modernStyleAction_->setShortText(
        *config_.modernStyle ? _("Modern Style: Bật") : _("Modern Style: Tắt"));
    if (ic) {
...
void vmkEngine::updateFreeMarkingAction(InputContext *ic) {
    ...
    freeMarkingAction_->setShortText(
        *config_.freeMarking ? _("Free Marking: Bật") : _("Free Marking: Tắt"));
    if (ic) {
...
std::string SubstrChar(const std::string &s, size_t start, size_t len) {
    ...
    const char *start_ptr =
        fcitx_utf8_get_nth_char(s.c_str(), static_cast<uint32_t>(start));
    if (*start_ptr == '\0')
        return "";
    if (len == std::string::npos)
        return std::string(start_ptr);
    const char *end_ptr =
        fcitx_utf8_get_nth_char(start_ptr, static_cast<uint32_t>(len));
...
int compareAndSplitStrings(const std::string &A, const std::string &B,
    ...
    for (size_t i = 0; i < minLength; ++i) {
        const char *ptrA =
            fcitx_utf8_get_nth_char(A.c_str(), static_cast<uint32_t>(i));
        const char *ptrB =
            fcitx_utf8_get_nth_char(B.c_str(), static_cast<uint32_t>(i));
